...
export const tapPlaceComponent = {
  ...
  tagLatestSpawnedAlmond(meta) {
    ...
    try {
      const all = this.el.sceneEl.querySelectorAll('a-entity[gltf-model="#cactusModel"]')
  ...
  onCactusSelected(event) {
    ...
    try {
      if (this.selectedCactus && this.selectedCactus.dataset) {
        if (this.selectedCactus.dataset.claimed !== '1') {
          const pts = Number(this.selectedCactus.dataset.points || 0)
          if (Number.isFinite(pts) && pts > 0) {
            this.selectedCactus.dataset.claimed = '1'
            this.score += pts
